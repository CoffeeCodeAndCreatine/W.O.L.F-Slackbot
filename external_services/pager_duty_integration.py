...
def get_pager_info_message_string(message):
    ...
    usernames = get_pager_usernames(pager_secrets.get("pager_duty_token"), pager_secrets.get("pager_duty_primary_ids"),
                                    pager_secrets.get("pager_duty_secondary_ids"))
    message_string = "Here are the users currently on support:\n```\n"
    message_string = message_string + "Primary support: " + usernames.get(
...
def get_pager_usernames(token, primary_ids, secondary_ids):
    ...
    secondary_requests = requests.get(pager_duty_url.format(secondary_ids[0], secondary_ids[1]),
                                      headers=pager_duty_headers)
